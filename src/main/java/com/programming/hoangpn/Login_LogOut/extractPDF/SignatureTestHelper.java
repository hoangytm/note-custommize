...
public class SignatureTestHelper {
    ...
    public Signature getSignatureInfor(MultipartFile uploadFile) throws IOException, GeneralSecurityException {
        ...
        Signature signature = Signature.builder()
                ...
                .build();
        log.info("H- signature: " + signature.toString());
//        SignedDocumentInfo signedDocumentInfo = signatureTestHelper.collectInfo(EXAMPLE);
        return signature;
    }
    ...
    private List<SignatureInfo> verifySignatures(SignatureUtil signUtil, List<String> names) throws IOException, GeneralSecurityException {
        List<SignatureInfo> signatureInfos = new ArrayList<>();
        for (String name : names) {
            SignatureInfo signatureInfo = new SignatureInfo();
            PdfPKCS7 pkcs7 = signUtil.readSignatureData(name);
            signatureInfo.setSignatureName(name);
            signatureInfo.setSignerName(pkcs7.getSignName());
            signatureInfo.setSignDate(pkcs7.getSignDate().getTime());
            ...
            if (!pkcs7.verifySignatureIntegrityAndAuthenticity()) {
                ...
                log.error("H- signature invalid");
            }
            signatureInfo.setDocumentModified(!pkcs7.verifySignatureIntegrityAndAuthenticity());
            verifyCertificates(pkcs7, signatureInfo);
...
//        initKeyStoreForVerification(ks);
        ...
        for (int i = 0; i < certs.length; i++) {
            ...
            checkCertificateInfo(cert, cal.getTime(), pkcs7, certificateInfo);
    ...
    private void checkCertificateInfo(X509Certificate cert, Date signDate, PdfPKCS7 pkcs7, CertificateInfo certificateInfo) throws GeneralSecurityException {
        try {
            certificateInfo.setIssuer(cert.getIssuerX500Principal());
            certificateInfo.setSubject(cert.getSubjectX500Principal());
            certificateInfo.setValidTo(cert.getNotAfter());
            certificateInfo.setValidFrom(cert.getNotBefore());
    ...
    private Boolean checkRevocation(PdfPKCS7 pkcs7, X509Certificate signCert, X509Certificate issuerCert, Date date) {
        try {
